...
public class CommandLegend implements CommandExecutor {
        ...
        if (cmd.getName().equalsIgnoreCase("legend")) {
            ...
            if (args.length > 0 && !args[0].equalsIgnoreCase("help")) {
                if (args.length > 1) {
                    ...
                    if (args[0].equalsIgnoreCase("getm")) {
                        MaterialObj item = materials.get(args[1]);
                        if (item == null) {
                            player.sendMessage(S.toPrefixRed("材料不存在"));
                            return true;
                        }
                        player.getInventory().addItem(item.getItem());
                        player.sendMessage(S.toPrefixGreen("成功获得 " + args[1]));
                ...
                if (args[0].equalsIgnoreCase("lists")) {
                    ...
                    if (result.length() == 2) {
                        player.sendMessage(S.toPrefixRed("没有套装"));
                        return true;
                    }
                    player.sendMessage(S.toPrefixGreen("套装列表 " + result.substring(1, result.length() - 1)));
                    return true;
                }
                if (args[0].equalsIgnoreCase("listm")) {
                    String result = materials.keySet().toString();
                    if (result.length() == 2) {
                        player.sendMessage(S.toPrefixRed("没有材料"));
                        return true;
                    }
                    player.sendMessage(S.toPrefixGreen("材料列表 " + result.substring(1, result.length() - 1)));
                    return true;
                }
                if (args[0].equalsIgnoreCase("update")) {
                    new UpdateItems().runTask(LegendItems.plugin);
    ...
    private void sendHelp(CommandSender player, String label) {
        ...
        player.sendMessage(S.toBoldGreen("/" + label + " getm <材料名>") + S.toGray(" - ") + S.toGold("获得传奇材料"));
        ...
        player.sendMessage(S.toBoldGreen("/" + label + " listm") + S.toGray(" - ") + S.toGold("查看传奇材料列表"));
